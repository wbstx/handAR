...
def augmentation(img, mask, tex_mask, bbox, data_split, exclude_flip=False):
    if data_split == 'train':
        scale, rot, color_scale, do_flip = get_aug_config(exclude_flip)
    ...
    img, trans, inv_trans = generate_patch_image(img, bbox, scale, rot, do_flip, cfg.input_img_shape)
    mask, _, _ = generate_patch_image(mask, bbox, scale, rot, do_flip, (224, 224))
    tex_mask, _, _ = generate_patch_image(tex_mask, bbox, scale, rot, do_flip, (224, 224))
    ...
    img = np.clip(img * color_scale[None, None, :], 0, 255)
    return img, mask, tex_mask, trans, inv_trans, rot, do_flip
