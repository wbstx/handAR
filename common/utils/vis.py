...
def vis_mesh(img, mesh_vertex, alpha=0.5):
    ...
    for i in range(len(mesh_vertex)):
        ...
        mask = cv2.circle(mask, p, radius=2, color=colors[i], thickness=-1, lineType=cv2.LINE_AA)

    # Blend the keypoints.
    return cv2.addWeighted(img, 1.0 - alpha, mask, alpha, 0)


def vis_normal(img, mesh_vertex, mesh_normal, alpha=0.5):
    # Convert from plt 0-1 RGBA colors to 0-255 BGR colors for opencv.
    cmap = plt.get_cmap('rainbow')
    ...
    # Draw the mesh
    for i in range(len(mesh_vertex)):
        if abs(np.arccos(mesh_normal[i][2]) - np.pi/2) < 0.1:
            p = mesh_vertex[i][0].astype(np.int32), mesh_vertex[i][1].astype(np.int32)
            mask = cv2.circle(mask, p, radius=2, color=colors[0], thickness=-1, lineType=cv2.LINE_AA)
